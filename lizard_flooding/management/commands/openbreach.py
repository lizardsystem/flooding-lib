...
class Scenario:
    ...
    def compute_sobek_model(self):
        ...
        if self.breach.externalwater.type == DB_SEA:
         ...
         ##################################################################
            if self.scenario.cutofflocations.all():
                log.info("alter the canal model for cutoff locations")
            ...
            for loc_cutoff in self.scenario.scenariocutofflocation_set.all():
                ...
                if loc_cutoff.cutofflocation.externalwater_set.count() > 0:
                    log.info('external cuttofflocation')
                    pool_ref = pool2
                    ...
                    log.info('internal cuttofflocation')
                    pool_ref = self.pool
        ...
        ##########################
            ...
            #network.gr (grid)
            branch_ext = sobek.Object("GRID id '%(branch_peak)s' ci '%(branch_peak)s' gr gr 'GridPoint Table' PDIN 0 0 '' pdin CLTT 'Location' '1/R' cltt CLID '' '' clid grid" % self.idpool)
            ...
            branch_ext = sobek.Object("GRID id '%(branch_mean)s' ci '%(branch_mean)s' gr gr 'GridPoint Table' PDIN 0 0 '' pdin CLTT 'Location' '1/R' cltt CLID '' '' clid grid" % self.idpool)
        ...
        # network.gr
        ngrid = sobek.Object("GRID id '%(grid)s' ci '%(breach)s' gr gr 'GridPoint Table' PDIN 0 0 '' pdin CLTT 'Location' '1/R' cltt CLID '' '' clid grid" % self.idpool)
...
def compute_sobek_model(scenario):
...
#??? old code ???
'''
    log.info("collect parameters from scenario")

    curs = connection.cursor()

    curs.execute("""\
SELECT scenarios.bottomlevelbreach as profile_depth,
       scenarios.widthbrinit AS initial_breach_width, 
	   scenarios.brdischcoef AS discharge_coefficient, 
	   scenarios.brf1 AS factor_1_alfa, 
	   scenarios.brf2 AS factor_2_beta, 
	   scenarios.bottomlevelbreach AS lowest_crest_level, 
	   scenarios.ucritical AS critical_flow_velocity, 
	   scenarios.pitdepth AS pit_depth, 
       TIMESTAMP '2000-01-01 00:00:00' as start_of_simulation,
	   scenarios.tstartbreach AS time_of_breach_start, 
	   scenarios.tmaxdepth AS time_of_breach_max_depth, 
	   scenarios.tsim AS duration_of_simulation, 
	   breaches.internalnodeid AS extern_id, 
	   transform(breaches.internalnode,28992) AS intern, 
	   inundationmodels.modelcase AS case, 
	   inundationmodels.fileloc_sobekproject AS sobek_project_dir, 
	   sobekversions.name AS sobek_version, 
	   extwaters.area AS extwaters_area, 
	   extwaters.extwtypes_fk AS extwtypes_id 
FROM public.scenarios, public.breaches, public.inundationmodels, 
     public.sobekversions, public.extwaters, public.extwtypes
WHERE scenarios.breach_fk = breaches.id 
  AND scenarios.inundationmodel_fk = inundationmodels.id 
  AND inundationmodels.sobekversion_fk = sobekversions.id 
  AND breaches.extwater_fk = extwaters.id 
  AND scenarios.id = %d""" % scenario)

    field_names = [i[0] for i in curs.description]
    try:
        params = dict(zip(field_names, curs.fetchone()))
    except TypeError:
        raise ValueError("scenario %d does not exist" % scenario)

    log.debug("get region/breach ids from database...")
    curs.execute("""\
SELECT scenarios.id AS scenario_id, 
       region_fk AS region_id, 
       breach_fk AS breach_id
FROM scenarios, breaches
WHERE scenarios.breach_fk = breaches.id
  AND scenarios.id = %d""" % scenario)
    field_names = [i[0] for i in curs.description]
    params.update(dict(zip(field_names, curs.fetchone())))

    try:
        curs.execute("""\
SELECT transform(breaches.externalnode,28992) AS extern
FROM public.scenarios, public.breaches
WHERE scenarios.breach_fk = breaches.id 
  AND scenarios.id = %d""" % scenario)
        field_names = [i[0] for i in curs.description]
        params.update(dict(zip(field_names, curs.fetchone())))
...
FROM scenariowaterleveltbl 
WHERE scenario_fk=%d""" % scenario)
    water_levels = dict(curs.fetchall())
    log.debug("water_levels: %s" % water_levels)

    if water_levels:
        self.max_water_level = max(water_levels.values())
        self.min_water_level = min(water_levels.values())

    curs.execute("SELECT name, value FROM settings")
    settings = dict(curs.fetchall())
    params.update(settings)

    curs.execute("SELECT id, name AS extwtypes_name FROM extwtypes")
    extwtypes = dict(curs.fetchall())

    try:

        curs.execute("""\
SELECT scenarios.extwbaselevel , -- breachlinkproperty.extwbaselevel
       bosommodels.fileloc_sobekproject AS sobek_project_dir, -- breachlinkproperty.sobekmodel_externalwater.project_fileloc
       bosommodels.modelcase AS case, -- breachlinkproperty.sobekmodel_externalwater.model_case
       sobekversions.name AS sobek_version -- breachlinkproperty.sobekmodel_externalwater.sobekversion.name
FROM public.scenarios, public.bosommodels, public.sobekversions 
WHERE scenarios.bosommodel_fk = bosommodels.id 
  AND bosommodels.sobekversion_fk = sobekversions.id 
  AND scenarios.id = %d""" % scenario)
        field_names = [i[0] for i in curs.description]
        canal_params = dict(zip(field_names, curs.fetchone()))
        canal_params.update(settings)

        curs.execute("""\
SELECT loccutoffs.sobeknodeid AS node_id, -- [self.scenario.scenariocutofflocation_set].cutofflocation.cutofflocationsobekmodelsetting_set.filter(sobekmodel=self.breachlinkproperty.sobekmodel_externalwater_id)
       scenario_loccutoffs.tclose AS delay_of_cutoff, -- [self.scenariocutofflocation_set].tclose
       loccutoffs.bottomlevel AS crest_level, -- [self.scenariocutofflocation_set].cutofflocation.bottomlevel
       loccutoffs.width -- [self.scenariocutofflocation_set].cutofflocation.width
FROM public.scenario_loccutoffs, public.scenarios, public.loccutoffs 
WHERE scenario_loccutoffs.scenario_fk = scenarios.id 
  AND scenario_loccutoffs.loccutoff_fk = loccutoffs.id 
  AND scenarios.id = %d""" % scenario)

        cutoff = {}
        field_names = [i[0] for i in curs.description]
        for i in curs.fetchall():
            item = dict(zip(field_names, i))
            cutoff[item['node_id']] = item

        canal_params['location_cutoff'] = cutoff
    ...
    intern = (self.intern.getX(), self.intern.getY())
    self.intern = intern
    if self.extern:
        extern = (self.extern.getX(), self.extern.getY())
        self.extern = extern
    else:
        if not self.extern_id:
            raise ValueError("must specify one of 'extern', 'extern_id'")

    log.debug("scenario: %s" % scenario)
    log.debug("canal_params: %s" % canal_params)
    log.debug("params: %s" % params)
    log.debug("water_levels: %s" % water_levels)

    if self.extern and self.extern_id:
        raise ValueError("cannot specify both of 'extern', 'extern_id'")
'''
