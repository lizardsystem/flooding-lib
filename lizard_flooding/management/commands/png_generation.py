...
def common_generation(connection, scenario_id, tmp_dir, source_programs):
    ...
    if os.stat(os.path.join(abs_output_dir_name, def_container))[stat.ST_SIZE] == 0:
        log.warning("input file '%s' is empty" % def_container)
    else:
        input_file = zipfile.ZipFile(os.path.join(abs_output_dir_name, def_container))
    ...
    for result in results:
        ...
        cm_content = file(os.path.join(cm_location, result.resulttype.color_mapping_name)).read()
        output = file(os.path.join(abs_output_dir_name, result.resulttype.name, 'colormapping.csv'), 'w')
        ...
        log.debug("place a copy of the colormapping too in the temporary directory.")
        output = file(os.path.join(tmp_dir, result.resulttype.name, result.resulttype.color_mapping_name), 'w')
        output.write(cm_content)
        output.close()

        # invoke part that will generate the png(s) based on the asc/inc files.
        infile_asc = compute_png_files(result, abs_output_dir_name, os.path.join(tmp_dir, result.resulttype.name + '/'), def_grid)
...
def compute_png_files(result, abs_output_dir_name, tmp_dir, def_grid):
    ...
    log.info("will produce png files out of %s, using %s color mapping." % (result.resulttype.content_names_re, result.resulttype.color_mapping_name))
    ...
    log.debug("open the ColorMapping %s." % (os.path.join(tmp_dir,result.resulttype.color_mapping_name)))
    colors = nens.asc.ColorMapping(file(os.path.join(tmp_dir, result.resulttype.color_mapping_name)))
