...
from lizard.flooding.models import Breach, CutoffLocation, CutoffLocationSet, \
...
def service_get_result_settings(
    ...
    settings = Setting.objects.get(  key = 'destination_dir'  )
    png_name = settings.value + "\\" + result.resultpngloc
...
def service(request):
    ...
    if request.method == 'GET':
        ...
        if action_name == 'get_projects':
            ...
            from views_dev import service_compose_scenario
            breach_id= query.get('breach_id', None)
            ...
            from views_dev import get_externalwater_graph
            mid = 24*60*60*1000
            ...
            from views_dev import service_save_new_scenario
