    ...
    for csvfile in glob.glob(os.path.join(options.csvpath, '*.csv')):
        ...
        try:
            ...
            if gridsize >-1 and exportid >-1:             
                success = os.system(options.pythonpath + ' ' + options.mwdtool + ' ' + csvfile + ' ' + options.dikes + ' ' + options.provinces + ' ' + str(gridsize) + ' ' + str(exportid))                    
        except Exception, e:            
...
if __name__ == '__main__':
    ...
    parser.add_option('--pythonpath', help='pythonpath for the export max waterdepth tool')
    parser.add_option('--mwdtool', help='path for the export max waterdepth tool')
    parser.add_option('--csvpath', help='the path to the folder where the csv files are stored')
    parser.add_option('--dikes', help='the shapefile describing the dikes of the Netherlands')
    parser.add_option('--provinces', help='the shapefile describing the borders of the provinces of the Netherlands')
    
    (options, args) = parser.parse_args()
    if not (options.pythonpath and options.mwdtool and options.csvpath and options.dikes and options.provinces):
        parser.print_help()
    else:
        main(options, args)
