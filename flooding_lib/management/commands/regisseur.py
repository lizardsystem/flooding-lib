...
class regisseur(threaded_program.threaded_program):
    ...
    def send_warning(self, subject, body):
        ...
        import email
        import socket
        me = ('regisseur@' + socket.gethostname().lower() +
              ".nelen-schuurmans.nl")
        ...
        while True:
            ...
            for task_executor in task_executors:
                ...
                proxy = xmlrpclib.ServerProxy(
                    "http://%s:%d" %
                    (task_executor.ipaddress, task_executor.port))
                try:
                    log.debug("remote %i returned: %s" %
                              (task_executor.id, proxy.status()))
                    log.info("uitvoerder %i is still alive" % task_executor.id)
                    status[task_executor.id] = 0
                except Exception, e:
                    task_executor.message = str(e)
                    log.warning(
                        "uitvoerder %s died with message %s" %
                        (task_executor.ipaddress, task_executor.message))
            ...
            if failing:
                mark_dead = []
                lines = [
                    'found uitvoerders which were marked as active and which '
                    'do not serve xmlrpc calls...  they may be dead in fact.  '
                    'please have a look!',
                    '',
                ...
                for task_executor in failing:
                    if status[task_executor.id] == 2:
                        lines.append(
                            ("| %(id)s | %(name)s | http://%(ipaddress)s"
                             ":%(port)d | %(message)s |")
                            % task_executor.__dict__)
                ...
                if mark_dead:
                    for task_executor in mark_dead:
                        ...
                        task_executor.save()
                    self.send_warning(
                        'regisseur found dead uitvoerders', '\n'.join(lines))
                pass
    ...
    def thread_watch_lost(self):
        ...
        while True:
            ...
            hanging = Task.objects.filter(
                Q(tfinished=None),
                Q(successful=None),
                tasktype__in=(
                    120, 130, 132, 134, 150, 155, 160, 162, 180, 185),
            ...
            if hanging:
                for task in hanging:
                    ...
                    task.remark = (
                        'regisseur (Rev: %s) marked this task as finished.' %
                        __revision__)
                    task.successful = False
                    task.save()

                log.info(
                    "marked lost tasks %s as free" %
                    str(hanging.values_list('id', flat=True)))
                lines = [
                    'found lost tasks in database and marked them back '
                    'as available.',
                    '',
                    "'|'-separated representation of the data.",
                    "| *id* | *scenario_fk* | *type_fk* | "
                    "*taskby* | *tstart* |"
                    ]
                for task in hanging:
                    lines.append(
                        ("| %(task_id)s | %(scenario_id)s | "
                        "%(tasktype_id)s | %(creatorlog)s | %(tstart)s |") %
                        task.__dict__)
                self.send_warning(
                    'regisseur found lost tasks', '\n'.join(lines))
                pass
            django.db.transaction.commit_unless_managed()
        ...
        while True:
            ...
            if failing:
                for d in failing:
                    d['now'] = datetime.now()
                    d['remarks'] = (
                        "regisseur (Rev: %s) parked task after %d failures" %
                        (__revision__, d['failures']))
                    curs.execute("""
                        INSERT INTO flooding_task
                            (scenario_id, tasktype_id,
                             creatorlog, tstart,
                             tfinished, successful, remarks)
                        VALUES
                            (%(scenario_id)s, %(tasktype_id)s,
                             'regisseur', %(now)s,
                             %(now)s, NULL, %(remarks)s)""",
                                 d)
                log.info(
                    "marked failing tasks %s as parked" %
                    str(["%(scenario_id)s-%(tasktype_id)s" %
                         i for i in failing]))

                lines = ['found failing tasks in database',
                         '',
                         ("partial '|'-separated representation of "
                          "the rows just added."),
                         ("| *scenario_fk* | *type_fk* | *tstart* | "
                          "*remarks* |"),
                         ]
                for d in failing:
                    lines.append(
