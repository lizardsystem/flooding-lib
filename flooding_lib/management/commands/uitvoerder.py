...
if sys.version_info < (2, 4):
    print (
        "I think I need version python2.4 and I was called from %d.%d" %
        sys.version_info[:2])
...
def geometric(p):
    ...
    if p <= 0.0 or p > 1.0:
        ...
        # If p is exactly 1.0, then the only possible generated value
        # is 1.  Recognizing this case early means that we can avoid a
        # log(0.0) later.  The exact floating point comparison should
        # be fine. log(eps) works just dandy.
        return 1
    ...
    # Find the corresponding geometric variate by inverting the
    # uniform variate.
...
class uitvoerder(threaded_program.threaded_program):
    def __init__(
        self, connection, capabilities, ip, port=8000, hostname=None, seq=None,
        scenario_range=[0, -1],
    ...
    def thread_loop(self):
        ...
        while True:
            self.sleep(
                "main executing loop starts by waiting five second", 5, 0)
            ...
            if choice is None:
                ...
                log.info(
                    "task %s-%s(%s): starting" %
                    (self.scenario, self.task, self.last_id))

                success_code, remarks, error_messages = perform_task(
                    self.scenario,
                    self.task,
                    self.sequential,
                    self.max_hours,
                ...
                time.sleep(10)
                log.info(
                    "task %s-%s(%s): successful=%s" %
                    (self.scenario, self.task, self.last_id, success_code))
                self.close_task(
                    successful=success_code, remarks=remarks + error_messages)
                django.db.transaction.commit_unless_managed()
                scen = flooding_lib.models.Scenario.objects.get(
                    pk=self.scenario)
                scen.update_status()
    ...
    def close_task(self, successful, remarks=None):
        #get cursor at begin of function (otherwise there is nothing
        ...
        curs.execute("""
        UPDATE flooding_task
        SET tfinished=%(finished_at)s,
    ...
    def choose_and_lock(self):
        ...
        task_dependencies = [
            {'candidate': TASK_COMPUTE_SOBEK_MODEL_120,
             'depends_on': TASK_ADMIN_CREATES_SCENARIO_050},
            {'candidate': TASK_PERFORM_SOBEK_SIMULATION_130,
             'depends_on': TASK_COMPUTE_SOBEK_MODEL_120},
            {'candidate': TASK_COMPUTE_RISE_SPEED_132,
             'depends_on': TASK_PERFORM_SOBEK_SIMULATION_130},
            {'candidate': TASK_COMPUTE_MORTALITY_GRID_134,
             'depends_on': TASK_COMPUTE_RISE_SPEED_132},
            {'candidate': TASK_HISSSM_SIMULATION_160,
             'depends_on': TASK_COMPUTE_MORTALITY_GRID_134},
            {'candidate': TASK_SOBEK_EMBANKMENT_DAMAGE_162,
             'depends_on': TASK_HISSSM_SIMULATION_160},
            {'candidate': TASK_SOBEK_PNG_GENERATION_150,
             'depends_on': TASK_PERFORM_SOBEK_SIMULATION_130},
            {'candidate': TASK_HISSSM_PNG_GENERATION_180,
             'depends_on': TASK_SOBEK_EMBANKMENT_DAMAGE_162},
            {'candidate': TASK_HISSSM_PRESENTATION_GENERATION_185,
             'depends_on': TASK_HISSSM_PNG_GENERATION_180},
            {'candidate': TASK_SOBEK_PRESENTATION_GENERATION_155,
             'depends_on': TASK_SOBEK_PNG_GENERATION_150},
            ]
        ...
        for dependency in task_dependencies:
            if dependency['candidate'] not in self.capabilities:
                continue
            curs.execute("""
    SELECT s.calcpriority,
           %(candidate)s,
           t.tfinished,
           t.scenario_id
    AS type_fk
    ...
    AND (t.tasktype_id=%(depends_on)s
    AND NOT (t.tfinished IS NULL)
    AND t.successful=TRUE
    AND NOT t.scenario_id IN
      (SELECT scenario_id
       FROM flooding_task
       WHERE tasktype_id=%(candidate)s
       AND (successful=TRUE OR successful IS NULL)))
    """, dependency)
            waiting = curs.fetchall()
            if low != 0:
                waiting = [
                    (p, c, f, s) for (p, c, f, s) in waiting if s >= low]
            if high != -1:
                waiting = [
                    (p, c, f, s) for (p, c, f, s) in waiting if s <= high]
            log.debug(
                "candidate-> %s: %s" % (dependency['candidate'], waiting))
        ...
        while True:
            ...
            if index_of_chosen_task < len(waiting_tasks):
                break
        chosen = waiting_tasks[index_of_chosen_task]
        _, params['task_type'], _, params['scenario_id'] = chosen
    ...
    VALUES
    (%(task_id)s, %(scenario_id)s, %(task_type)s,
    %(taskby)s, %(started_at)s, %(remarks)s)""",
                     params)
...
def main(args, options):
    ...
    try:
        if options['sequential']:
            curs.execute(
                """SELECT id, seq
                   FROM flooding_taskexecutor
                   WHERE name=%(hostname)s AND seq=%(sequential)s""",
                         options)
        else:
            curs.execute("""SELECT id, seq
                            FROM flooding_taskexecutor
        ...
        raise ValueError(
            ("database contains no taskexecutor where name[/seq]="
             "(name=%(hostname)s AND seq=%(sequential)s)") % options)
    if curs.fetchone():
        raise ValueError(
            ("database contains multiple uitvoerders matching name/"
             "seq=%(hostname)s/seq=%(sequential)s") % options)

    if options['capabilities']:
        log.debug("capabilities were specified: update the database")
        curs.execute("""
            DELETE FROM flooding_taskexecutor_tasktypes
...
WHERE taskexecutor_id=%(uit_id)s""",
    ...
    if options['x_port']:
        ...
        curs.execute("""\
            UPDATE flooding_taskexecutor
            SET port=%(x_port)s WHERE id=%(uit_id)s""",
                      options)
        django.db.transaction.commit_unless_managed()
        pass
    else:
    ...
    if options['capabilities'] == []:
        raise ValueError(
            "uitvoerder is not able of doing anything - not starting")

    log.info(("starting uitvoerder %(uit_id)s:%(hostname)s[%(sequential)s]"
              " on tasktypes %(capabilities)s - available at "
              "http://%(x_ip)s:%(x_port)s") % options)
...
""")
    ...
    make_option(
            '--quiet', help='log warnings and errors',
    ...
    make_option(
            '--hisssm-location', default='C:/Program Files/HIS-SSMv2.4/',
            help='the root of the his-ssm installation'),
    make_option(
            '--hisssm-year', default=2008,
            help='the year of hisssm simulation data', type='int'),
    make_option(
            '--x-port',
            help='the xmlrpc server port (if not given, taken from db)',
            default=None, type='int'),
    make_option(
            '--capabilities', help='tasks that uitvoerder can perform',
            default=[], action='append', type='int'),

    make_option(
            '--scenario-range', help='DEBUG: limit action on these scenarios',
            nargs=2, type='int', default=[0, -1]),
    make_option(
            '--sequential',
            help=('use this if you need more than one uitvoerder '
                  'on this workstation'),
            type='int', default=None),

    make_option(
            '--tmp_location', default='C:/temp/',
            help='the root of the temporary files for task execution'),
    make_option(
            '--max_hours', help='maximum hours for a single task ',
            default=72, type='int'))
