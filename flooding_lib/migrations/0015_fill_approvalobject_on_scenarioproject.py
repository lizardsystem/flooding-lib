...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """approvalobject is now a field on ScenarioProject. Fill it
        with the data from the old field, on Scenario."""

        for scenario in orm.Scenario.objects.all():
            if not scenario.approvalobject:
                continue

            # This could throw a DoesNotExist exception, but that
            # shouldn't happen so we just let the migration crash in
            # that case.
            scenarioproject = orm.ScenarioProject.objects.get(
                scenario=scenario, is_main_project=True)

            scenarioproject.approvalobject = scenario.approvalobject
    ...
    def backwards(self, orm):
        ...
        for scenario in orm.Scenario.objects.all():
            ...
            scenario.approvalobject = scenarioproject.approvalobject
            scenario.save()
