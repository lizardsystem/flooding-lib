...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """ Create ResultType objects for 3Di scenarios. """
        ResultType = orm.ResultType

        # shared defaults
        shared_defaults = {'program': orm.Program.objects.get(name='import')}

        name = 'results_3di'
        defaults = {
            'shortname_dutch': '3Di resultaat',
            'content_names_re': 'results_3di.nc',
        }
        defaults.update(shared_defaults)
        ResultType.objects.get_or_create(name=name, defaults=defaults)

        name = 'bathymetry'
        defaults = {
            'shortname_dutch': 'Bathymetrie',
            'content_names_re': 'bathymetry.tif',
        }
        defaults.update(shared_defaults)
        ResultType.objects.get_or_create(name=name, defaults=defaults)
