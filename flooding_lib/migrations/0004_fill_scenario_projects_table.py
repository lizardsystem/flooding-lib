...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Translate all foreign keys from Scenario to Project to the
        new format."""

        for scenario in orm.Scenario.objects.all():
            sp = orm.ScenarioProject(
                scenario=scenario,
    ...
    def backwards(self, orm):
        """Take only the main ones and set them in the foreign key field."""

        for scenarioproject in orm.ScenarioProject.objects.filter(
            is_main_project=True):
            scenario = scenarioproject.scenario
            scenario.project = scenarioproject.project
            scenario.save()
