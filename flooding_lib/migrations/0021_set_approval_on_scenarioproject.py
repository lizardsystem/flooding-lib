...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for scenarioproject in (
            orm['flooding_lib.ScenarioProject'].objects.all()):
            if scenarioproject.approvalobject:
                approvalobject = scenarioproject.approvalobject
                rules = orm['approvaltool.ApprovalObjectState'].objects.filter(
                    approvalobject=approvalobject)

                if all(rule.successful for rule in rules):
                    scenarioproject.approved = True
                    scenarioproject.save()
                elif any(rule.successful == False for rule in rules):
                    scenarioproject.approved = False
                    scenarioproject.save()
                # Else keep it None
    ...
    complete_apps = ['approvaltool', 'flooding_lib']
