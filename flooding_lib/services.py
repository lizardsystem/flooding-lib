...
from flooding_lib.views_dev import service_compose_scenario, get_externalwater_graph, get_externalwater_graph_session, get_externalwater_graph_infowindow, service_save_new_scenario, get_externalwater_csv, service_select_strategy
from flooding_base.models import Setting
from flooding_lib.models import Breach, CutoffLocationSet, \
...
def service_get_existing_embankments_shape(request, width, height, bbox, region_id, strategy_id, only_selected = False):
    ...
    lyr_specific_region.datasource = mapnik.PostGIS(host=database['HOST'],
                                                    user=database['USER'],
                                                    password=database['PASSWORD'],
                                                    dbname=database['NAME'],
                                                    table=str(BUFFERED_TABLE))
    ...
    lyr_new_embankments.datasource = mapnik.PostGIS(host=database['HOST'],
                                                    user=database['USER'],
                                                    password=database['PASSWORD'],
                                                    dbname=database['NAME'],
                                                    table=str(BUFFERED_TABLE))
    ...
    lyr_polygon_selected_embankments.datasource = mapnik.PostGIS(host=database['HOST'],
                                                                 user=database['USER'],
                                                                 password=database['PASSWORD'],
                                                                 dbname=database['NAME'],
                                                                 table=str(BUFFERED_TABLE))
